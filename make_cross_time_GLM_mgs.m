function [results]= make_cross_time_GLM_mgs(y,closest_surf_phi,tphi)
...
X1=double([ones(size(y,1),1) predictor]);
...
contrast.name={'Sacc-memory';};
 ...
 for i=1:size(y,2)
     disp(['GLM(1) crosstime ' num2str(i)])
     temp_y=y(:,i);
%      if rank(X1(~isnan(temp_y),:))==length(X1(isnan(temp_y),:))
%          continue
%      end
